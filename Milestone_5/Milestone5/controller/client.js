...
router.post("/", async function(request, response)  {
  if (request.session.token===undefined){   // for login post only
    ...
    let user = await userDB.verifyUser(email,passwd); // get the desired user or the first user if the one we want is not there
    if (user==null){
      request.theView = "InvalidUser";
      request.theData = {
      message: "Invalid Username or Password",
      };
      if (request.theView == "InvalidUser") {
        //console.log("Inside Request")
        data = request.theData;
